<div class='row'>
    ...
    <div class='col-md-6'>
        <div class="form-group">
            ...
            <select id="select-country" name='vendor_id' class='form-control' id='vendor_id'
                aria-describedby='vendor_id_help'>
                @foreach ($vendors as $vendor)
                <option @if(old("vendor_id")==$vendor->id) selected='selected' @endif value="{{$vendor->id}}">
                    {{$vendor->name}}
                </option>
        ...
        <div class="form-group">
            ...
            <input type="text" type="date" class="datepicker form-control" id="billed_at" aria-describedby="billed_at_help"
                name='billed_at' value="{{old("billed_at",$bill->billed_at ?? \Carbon\Carbon::now()->format('Y-m-d'))}}">
        </div>
    ...
    <div class='col-md-6'>
        <div class="form-group">
            ...
            <input type="text" class="form-control" id="bill_number" aria-describedby="bill_number_help"
                name='bill_number' value="{{old("bill_number",$bill->get_bill_number())}}">
        </div>
        <div class="form-group">
            <label for="due_at"> Due Date </label>
            <input type="datetime" class="datepicker form-control" name="due_at" aria-describedby="due_at_help" name='billed_at'
                value="{{old("due_at",$bill->billed_at ?? \Carbon\Carbon::now())}}">
        </div>
    </div>
</div>
<div class="card mt-4">
...
<div class="col-md-12 form-group">
    ...
    <textarea style="width: 100%" type="text" class="form-control" id="notes" name='notes'
        value="{{old("notes",$bill->notes)}}"></textarea>
</div>
<select class="name-input form-control d-none ">
    <option value="">Select a product name</option>

    @foreach ($products as $product)
    <option class="option-input " id="product-{{ $product->id }}" value='{{$product->id }}&{{$product->purchase_price }}'>
