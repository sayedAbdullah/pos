...
(function (jQuery) {
    ...
    if(paid.hasClass('hidden')){
        ...
        $('#status').on('change', function (e) {
            if($('#status').val() == 'partial'){
                paid.slideToggle(1000,);
                paid.removeClass('hidden');
            }else if(!paid.hasClass('hidden')){
                paid.slideToggle(1000);
                paid.addClass('hidden');
            }
        });

        $(".row").on('change','select.item', function (e) {
            var selected =  $(this).val();
             array = selected.replace(/^\[\'|\'\]$/g,'').split("&");
            var price = array['1'],
                id = array[0];

            var price = $.number(price, 2);
            var td = $(this).parents('.item-row').children('td');
            td.children('.price').val(price);
    ...
    $('.name-input').on('change', function (e) {
        ...
        var selected =  $("option[value="+val+"]");
        var name = selected.data('name');
        var id = selected.data('id');
        var price = $.number(selected.data('price'), 2);
        var total = price*2
...
Invoice.prototype = {
    ...
    newRow: function () {
        // // var options = jQuery.trim($('.name-input').html());

        console.log(options);



        jQuery(".item-row:last").after('<tr class="item-row"><td class="pl-4 item-name"><div class="delete-btn row"><a class=' + $.opt.delete.substring(1) + ' href="javascript:;" title="Remove row">X</a><select name="product[]" class="form-control-custom selectize item" placeholder="Select a country..." placeholder="Item">'+ options +'</select></div></td><td><input name="price[]" class="form-control price" placeholder="Price" type="text"> </td><td><input class="form-control qty" placeholder="Quantity" name="quantity[]" type="number" value="1"></td><td><span class="total">0.00</span></td></tr>');


        if (jQuery($.opt.delete).length > 0) {
            jQuery($.opt.delete).show();
        }
        jQuery(".item-row:last select").selectize();
